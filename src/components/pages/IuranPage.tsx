...
export function IuranPage({ notify }: { notify: (m: string, t?: "success" | "error") => void }) {
  const [data, setData] = useState<store.Iuran[]>([]);
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<store.Iuran | null>(null);

  const load = async () => {
    setData(await store.getAll<store.Iuran>("iuran"));
  };
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editing?.id) {
      await store.update("iuran", editing.id, payload);
    } else {
      await store.insert("iuran", payload);
    }
    setShowModal(false);
    setEditing(null);
