...
export function WhatsAppPage() {
  ...
  const [selected, setSelected] = useState<store.Warga[]>([]);
  const [message, setMessage] = useState("");
  const [search, setSearch] = useState("");

  useEffect(() => {
    store.seedIfEmpty().then(() => store.getAll<store.Warga>("warga")).then(setWarga);
  }, []);
  ...
  const toggleSelect = (w: store.Warga) => {
  ...
  const broadcast = () => {
    ...
    if (selected.length > 1) {
      alert(`Pesan akan dikirim ke ${selected.length} penerima. Browser akan membuka satu per satu.`);
    }
  };
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${
                    isSelected ? "bg-indigo-50 border-indigo-300" : "border-slate-100 hover:bg-indigo-50/30"
                  }`}
                >
                  <div>
