...
export function WargaPage({ notify }: { notify: (m: string, t?: "success" | "error") => void }) {
  const [data, setData] = useState<store.Warga[]>([]);
  const [search, setSearch] = useState("");
  const [editing, setEditing] = useState<store.Warga | null>(null);
  const [showModal, setShowModal] = useState(false);

  const load = async () => {
    const rows = await store.getAll<store.Warga>("warga");
    setData(rows);
  };
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editing?.id) {
      await store.update("warga", editing.id, payload);
    } else {
      await store.insert("warga", payload);
    }
    setShowModal(false);
    setEditing(null);
