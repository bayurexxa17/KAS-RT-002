...
export function Dashboard() {
  ...
  const stats = [
    { label: "Total Kas RT", value: data ? rupiah(data.saldo) : "Rp 0", icon: "fa-wallet", color: "indigo-500" },
    { label: "Jumlah Warga", value: data ? String(data.totalWarga) : "0", icon: "fa-users", color: "emerald-400" },
    { label: "Tagihan Pending", value: data ? String(data.tagihanPending) : "0", icon: "fa-clock", color: "orange-500" },
    { label: "Pemasukan Total", value: data ? rupiah(data.pemasukan) : "Rp 0", icon: "fa-chart-line", color: "cyan-400" },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {stats.map((s) => (
          <div key={s.label} className="soft-card p-6 card-shadow flex items-center justify-between">
            ...
            <div className={`w-12 h-12 bg-${s.color} rounded-2xl flex items-center justify-center text-white shadow-lg`}>
