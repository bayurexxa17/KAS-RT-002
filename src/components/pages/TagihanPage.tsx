...
export function TagihanPage({ notify }: { notify: (m: string, t?: "success" | "error") => void }) {
  const [data, setData] = useState<(store.Tagihan & { warga?: store.Warga })[]>([]);
  const [warga, setWarga] = useState<store.Warga[]>([]);
  const [iuranList, setIuranList] = useState<store.Iuran[]>([]);
  ...
  const load = async () => {
    const [t, w, i] = await Promise.all([
      store.getAll<store.Tagihan>("tagihan"),
      store.getAll<store.Warga>("warga"),
      store.getAll<store.Iuran>("iuran"),
    ]);
    setWarga(w);
    setIuranList(i);
  ...
  const toggleStatus = async (id: number, current: string) => {
    ...
    await store.update("tagihan", id, { status: newStatus } as any);
