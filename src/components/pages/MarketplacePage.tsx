...
export function MarketplacePage({ notify }: { notify: (m: string, t?: "success" | "error") => void }) {
  ...
  const load = () => api<Market[]>("/api/data?table=marketplace").then(setData);
  useEffect(() => {
    load();
  }, []);
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await api("/api/data?table=marketplace", {
      method: "POST",
  ...
  const handleDelete = async (id: number) => {
    ...
    await api(`/api/data?table=marketplace&id=${id}`, { method: "DELETE" });
