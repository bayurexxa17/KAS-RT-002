...
import { api, rupiah, formatDate } from "@/lib/utils";
...
type Warga = { id: number; nama: string };
...
export function TransaksiPage({
  ...
  const load = async () => {
    const [tx, w] = await Promise.all([
      api<Tx[]>(`/api/data?table=transaksi`),
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await api("/api/data?table=transaksi", {
      method: "POST",
  ...
  const handleDelete = async (id: number) => {
    ...
    await api(`/api/data?table=transaksi&id=${id}`, { method: "DELETE" });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`px-5 py-2 bg-${color}-500 text-white rounded-lg font-bold text-sm shadow-lg hover:bg-${color}-600 flex items-center gap-2`}
          >
